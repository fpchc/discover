...
├── src/
│   ├── main.tsx               应用入口（createRoot + BrowserRouter + Root 根层 Toaster + 全局错误边界 + 样式）
│   ├── App.tsx                应用路由表（/login、/、/conversations/:conversationId、/profile、/usage、其余回退 /；账号页路径级懒加载）
...
│   │   ├── conversations.ts   会话列表（后端 GET /conversations 为唯一事实源；显示级 localStorage 缓存 `disf_conversations_cache` 免刷新闪骨架，每次全量校准，登出清空）
│   │   ├── assistants.ts      助手目录 + 当前选择（agent_id）
│   │   ├── auth.ts            账号认证（status/account；resolveSession / login / logout / expire；登录写令牌对、登出调后端作废、过期清令牌对；登出重置 chat / conversations / assistants 防跨账号泄漏）
...
│   │   ├── AuthGate.tsx       认证闸门 + URL 守卫（loading → 非 /login 重定向 /login → authenticated 挂载路由；已登录在 /login 回退 /；main.tsx 包裹层）
│   │   ├── LoginScreen.tsx    登录页（/login；手机号 + 密码 → POST /auth/login）
│   │   ├── ChatPage.tsx       对话页（/、/conversations/:conversationId；侧栏 + 会话窗口 + 输入区，URL↔store 同步：URL 参数驱动打开/复位、新会话 ID 同步回 URL；useChatStream 挂载于此，离开即卸载清理）
│   │   ├── AccountLayout.tsx  账号页布局（/profile、/usage 共享左导航：NavLink 用量主区 + 个人中心左下，内容区 Outlet 懒加载）
│   │   ├── ProfilePage.tsx    个人中心（/profile 独立页；账号卡 + 头像更换 + 资料只读 + 修改密码）
│   │   ├── UsagePage.tsx      用量（/usage 独立页；聚合卡片 + ECharts 按日趋势图，echarts 独立 chunk）
│   │   ├── Sidebar.tsx        品牌左栏（品牌 / 新对话 Ctrl+K / 助手 / 最近对话 / 底部账号区入口 / 主题 / 退出；桌面折叠 → 64px 图标轨道）
│   │   ├── PageHeader.tsx     账号页内容顶栏（返回钮 → 回对话页 + 居中标题，h-14 对齐 ChatWindow）
...
| 路由 | **react-router v8**（`BrowserRouter`，纯客户端 SPA；`main.tsx` 包裹） | 五条页面路由：`/login` 登录、`/` 新对话、`/conversations/:conversationId` 会话页（URL 携带会话 ID）、`/profile` 个人中心、`/usage` 用量；`*` 回退 `/`。URL 为页面导航唯一事实源（切换会话 / 新对话 / 新会话 ID 创建均走 navigate 或同步回 URL；仅「新建会话」允许 store→URL 反向同步，复位/切换只由 URL 驱动，避免点「新对话」误跳回旧会话页）；AuthGate 壳外守卫（未登录 → `/login`，已登录在 `/login` → `/`）；nginx 已有 history 回退；账号页路径级懒加载，echarts 用量页独立 chunk |
| 会话恢复 | 纯 URL 深链 `/conversations/:id` 承担，无独立指针 store | 曾用 `stores/view.ts` + localStorage `disf_conversation_id` 在落地 `/` 时自动重开上次会话（`loadList` 异步 `.then` 后 navigate）——但该异步恢复会在用户点「新对话」后晚到而误跳回旧会话页（访问首页 / 点新对话均被劫持到旧会话）。现移除该自动恢复：落地 `/` 恒为新对话空态，恢复上次会话由 URL 深链承载（刷新于 `/conversations/:id` 即停留该会话，无需本地指针）；已删 `view.ts` / `view.test.ts`，登出经 `resetAppState` 只重置 chat / conversations / assistants |
...
| 历史数据源 | 后端历史接口（`GET /conversations`、`/conversations/{id}/messages`，见 `.claude/feature/API.md`） | 会话列表 / 消息以后端为唯一事实源；会话列表仅做**显示级 localStorage 缓存**（`disf_conversations_cache`）免刷新闪骨架，每次全量校准、登出清空；消息正文一律后端拉取；新会话乐观入列，回合结束后 `reconcileList` 校准 |
...
- **主包 728KB（gzip 231KB）**：已按路由拆懒加载——`UsagePage`（echarts）独立 chunk 540KB、`ProfilePage` 6KB、`AccountLayout` 3KB，仅访问 `/usage` 才加载 echarts；`chunkSizeWarningLimit` 已调至 800。若主包继续增大，可再拆 vendor 或按会话页懒加载。
...
阻塞模式兜底重试、Markdown 渲染（sanitize + 高亮、remark-breaks 单换行转 `<br>`、代码块语言标签头 + 复制钮）、
