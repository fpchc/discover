...
> 本文件只保留跨任务长期有效的规则；专项规范放 `.claude/specs/*.md`，当前架构事实放 `.ai/*.md`。

## 0. 规则层级与简化机制

优先级从高到低：平台安全与工具约束 → 本文件硬约束 → 约束范围内的用户当前明确要求 → 当前任务匹配的专项规范 → `.ai` 架构记忆 → 设计默认值。低优先级内容不得覆盖高优先级规则。
...
- 开始任务前，仅加载与任务直接匹配的 `.claude/specs/*.md`；不批量加载全部专项规范。归属不明确时先读取 `.claude/specs/INDEX.md`，再选择具体规范。多个规范冲突时按第 0 节处理，并指出冲突。
...
- `.ai/ARCHITECTURE.md` 是当前架构与关键决策的事实源；`.ai/MODULE_MAP.md` 是职责到路径的事实源。
- 只有代码结构、模块职责或关键设计决策发生变化时才更新 `.ai`；一次性报告和进度记录不得写入。
