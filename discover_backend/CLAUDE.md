...
* **对话边界 = 当前仓库目录**：全部工作与工具调用（Bash / Read / Glob / Grep 等）只允许落在当前工作目录内；禁止扫描 / 读取当前目录之外的任何路径（桌面、用户目录、图片、剪贴板、临时目录等）
* **DB 相关测试一律不做**：涉及数据库 / 读取环境变量（`.env`、真实 `os.environ`）的测试与命令禁止执行——读取环境变量与对话边界约束冲突；交付自检只跑无 DB 的 `tests/unit`
* 敏感文件：`.env`、`*.pem`、`*.key`
