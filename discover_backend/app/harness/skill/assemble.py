...
from app.environment.tools.plan import CapabilitySpec, ScriptSpec, ToolActivationPlan
...
class AssemblyPlan(BaseModel):
    ...
    def tool_plan(self) -> ToolActivationPlan:
        """投影为环境侧计划（唯一消费方 ToolBroker）。

        依赖方向固定为 harness → environment：环境不认识技能清单，装配层负责把
        技能声明翻译成环境能执行的形式。
        """
        return ToolActivationPlan(
            ...
            mcp_degrade_notes=dict(self.mcp_degrade_notes),
            capabilities=[
                CapabilitySpec(
                    capability=capability.capability,
                    strategy=capability.strategy,
                    candidate_servers=list(capability.candidate_servers),
                    core_tools=list(capability.core_tools),
                    required=capability.required,
                    degrade_note=capability.degrade_note,
                    fallback_capability=capability.fallback_capability,
                    fallback_servers=list(capability.fallback_servers),
                )
            ...
            scripts=[
                ScriptSpec(
                    ...
                    schema_path=script.schema_path,
                    timeout_seconds=script.timeout_seconds,
                    side_effect=script.side_effect,
                )
