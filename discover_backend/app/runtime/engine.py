...
from app.domain.assistant.models import AssistantTarget, SelectionSource, TargetType
...
class Runtime:
    ...
    def __init__(
        ...
        *,
        ...
        self._executor = ToolExecutor(self._broker, self._files)
    ...
    async def tool_node(self, state: GraphState) -> dict[str, object]:
        """工具执行节点：委托 ToolExecutor（分发/事件/门禁/产物登记），Engine 只做状态转移。"""
        outcome = await self._executor.execute(
            state.pending_calls,
            emitter=self._emit_sync_guard(),
            account_id=self._account_id,
