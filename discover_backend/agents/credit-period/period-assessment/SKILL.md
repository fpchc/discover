...
| L4 账期内风险 | 会不会爆雷？ | 企业风险能力（enterprise_risk）优先；企业风险无法覆盖的突发舆情/负面新闻以联网搜索（web_search）补充 |
...
按 `references/data-source-mapping.md` 的维度映射采集；数据能力由平台装配。**优先级：企业专有数据（天眼查等工商/风险数据）> 联网搜索（公开信息，仅兜底）**。企业专有数据已覆盖的字段不再查联网搜索，联网搜索仅用于企业专有能力无法覆盖的字段或全部不可用时的降级兜底。采集维度：
