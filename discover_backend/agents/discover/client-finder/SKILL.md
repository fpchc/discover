...
### 3.1 搜索与初筛（数据受限）
数据源由平台装配的数据能力提供。**优先级：企业专有数据能力（天眼查等工商/风险数据）> 联网搜索（公开信息，仅兜底）**。企业专有数据能力已覆盖的字段不再查联网搜索，联网搜索仅用于企业专有能力无法覆盖的字段或全部不可用时的降级兜底。多路关键词并发：产品名+行业、行业+区域、竞品+客户/供应链、招标/机会词。
...
- 附录 A 数据日志记录工具来源，突出实际使用的数据能力（企业专有能力为主，联网搜索仅兜底），让读者明确数据范围。
