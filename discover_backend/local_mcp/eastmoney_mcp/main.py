"""东方财富（eastmoney）MCP 服务（Streamable HTTP）：把东财资讯搜索暴露为标准 MCP 工具。
...
运行：python -m local_mcp.eastmoney_mcp.main（默认 127.0.0.1:10002/mcp）。
"""
...
SERVER_NAME = "eastmoney-search-mcp"
SERVER_VERSION = "0.1.0"

_TOOLS: list[dict[str, Any]] = [
    {
        "name": "web_search_eastmoney",
        "description": "东方财富资讯搜索：返回财经新闻标题、正文摘要、来源与链接（免费免鉴权）",
        "inputSchema": {
            "type": "object",
...
def _provider_factories(settings: EastmoneyMCPSettings) -> dict[str, SearchProvider]:
    return {"web_search_eastmoney": EastmoneySearchProvider(settings)}


class EastmoneyMcpLifespan:
...
async def _lifespan_factory(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    lifespan = EastmoneyMcpLifespan(providers)
    async with lifespan:
        yield
...
def _authorize(request: Request, settings: EastmoneyMCPSettings) -> None:
    """Bearer 令牌校验，fail-closed。"""
    token = settings.eastmoney_mcp_token
    if not token:
        raise HTTPException(status_code=401, detail="eastmoney_mcp_token 未配置")
...
def create_app(
    settings: EastmoneyMCPSettings | None = None,
    *,
    providers: dict[str, SearchProvider] | None = None,
) -> FastAPI:
    """装配东方财富 MCP 服务。测试可注入 providers（MockTransport 的 httpx 客户端）。"""
    settings = settings or EastmoneyMCPSettings()
...
def main() -> None:
    ...
    logger.info(
        ...
        f"{settings.eastmoney_mcp_host}:{settings.eastmoney_mcp_port}"
    )
    logger.info(
        f"MCP endpoint: http://{settings.eastmoney_mcp_host}:{settings.eastmoney_mcp_port}/mcp"
    )

    uvicorn.run(
        app,
        host=settings.eastmoney_mcp_host,
        port=settings.eastmoney_mcp_port,
        log_level="info",
    )
