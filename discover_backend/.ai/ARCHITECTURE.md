...
| **思考有正文无修复（2026-09-04）** | 专家会话只出思考不出正文的根因是 ReAct 触达硬预算（`agent_max_total_tokens=100000` / `agent_max_input_tokens=80000`）→ `PARTIAL_BUDGET` → 空 answer。修复六件套：① 默认预算上调（150000/120000，`.env`/`.env.example` 同步）；② MCP 泛化分发工具（`call_tool`/`call_tools_batch`）经 `mcp_hidden_tool_names` 从模型可见清单剔除；③ `role="tool"` 回传正文按 `agent_tool_message_max_chars` 截断；④ `PhaseExecutionRequest.thinking_enabled` 由全局开关与装配层 `thinking_preference` 共同决定，`off` 时关闭思考；⑤ `check_budget` 兑现 `finalization_reserve_tokens`（token 维度进入预留区即软性收尾，不再硬截断）；⑥ TurnRecorder 区分 `succeeded`/`partial` 落库（`MessageStatus.PARTIAL`），`message_end` 透出 status/reason/limitations | 用户决策 2026-09-04，实测验证：ruff/mypy 全绿、tests/unit 324 通过 |
