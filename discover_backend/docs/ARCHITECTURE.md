...
> 会话删除接口已按用户决策移除。**统一认证平台接管认证（2026-09-05）**：登录令牌
> 由平台签发，本项目只本地验签（HS256 + 共享密钥 + aud + iss + type=access）；
> 平台 user_id（JWT sub）**仅作登录映射**（auth_user_id find-or-create 本地账号），
> 对外标识与数据隔离列（from_account_id / created_by）**恒为本地账号 uuid 文本**，
> 不引入第二套用户标识；**兼容模式（2026-09-07）**：原本地登录（手机号+密码 /
> elecnest SSO / 刷新 / 登出 / 改密码）恢复可用作为兼容回退，受保护接口同时接受
> 平台令牌与本地令牌（本地令牌额外校验 Redis 会话）。
> 历史：手机号+密码 Argon2id + JWT + Redis 会话
> 为旧账号体系（2026-08-28，兼容回退恢复 2026-09-07）。脚本执行已去容器化（2026-08-21 用户决策）：
...
> 工作区 `workspaces/{account_id}/{conversation_id}/{run_id}` 按账号/会话/运行隔离。工具执行统一经 ActionGateway；候选结果经 Verifier 后决定终态；全套测试 179 通过
...
| 持久化 | PostgreSQL + SQLAlchemy(async) + Alembic；迁移唯一通道（改模型 → `autogenerate` → `upgrade`）；引擎连接池配置驱动（默认 QueuePool 复用连接，`db_pool_size<=0` 回退 NullPool） | 用户决策 2026-08，CLAUDE.md §1；2026-09-07 换连接池修复远程库保存延迟 |
| 对话历史落库 | conversations 会话头 + messages 回合明细（query/answer/thinking 一行，usage 聚合到回合）；`ConversationService.record_turn` 回合结束单次落库（**含中断回合**：客户端断开/取消/异常也落库，status=interrupted/error 保留 partial 内容；写入经 `CancelScope(shield=True)` 保护，不被取消传播打断），**DB 降级内部消化**（舱壁：失败记日志返回 bool，路由无 try/except、无 DB 感知）；首回合会话行由 record_turn 内部 upsert（name=截断首条 query，续聊保留）；会话接口 `GET /conversations`、`/messages`、`DELETE /conversations/{id}`（**软删除**标记独立 `is_delete=true`，业务状态 status 不被覆盖、行与 token 保留、仅列表隐藏、不可续聊；释放内存会话/运行时，两者皆无 → 404） | 用户决策 2026-08，评审采纳 |
...
| 目录结构 | 技能包三级 `agents/{agent}/{skill}`（去掉 skills/ 壳与 shared/）；工作区 `workspaces/{account_id}/{conversation_id}/{run_id}` 按账号/会话/运行隔离，会话删除不再清工作区 | 用户决策 2026-08 |
...
| **Agent Runtime V2 重构（2026-09-03）** | 依据 `react-runtime-v2-architecture.md`（替代 react-optimization/agent-architecture 两份旧方案）：**Workflow 决定当前执行哪个阶段，ReAct 只是阶段内 Bounded 策略**；Policy 在 LLM/工具前后施加确定性约束（§5.2「LLM 只有建议权、Engine 拥有控制权」）；Contract 判定阶段是否真正完成；Checkpoint 只在阶段/副作用边界持久化；生产组合根使用 PostgreSQL 快照/事件 + Redis 租约/取消/心跳；Event 是 HTTP/SSE/审计统一输出；Conversation/Run/Step 分离建模。**结构化决策通道 P0-1**：复用 ToolCall 通道，三个保留控制工具（`complete_phase`/`submit_final_answer`/`request_clarification`）经 `parse_decision` 归一（五规则，混合/多控制/纯文本→INVALID），控制工具不进 ToolBroker。**首期交付边界 P0-3 锁定单阶段内核**：契约+状态模型（`models.py`）→ Bounded ReAct 子图（`react/`+`graph.py`）→ Policy（`policy/`）→ Tool Runtime 管线（`execution/pipeline.py`）→ Contract（`contracts/`）→ Checkpoint+Run API（`checkpoint/`+`service.py`）。**短期共存**：V1 Runtime（engine/state/transition）仍跑现网，V2 模块独立可测（Fake LLM 图级测试跑通正常/死循环/交替无进展/预算硬边界/重复有新进展），W8 经 `agent_runtime_v2_enabled` 灰度切换后下线旧 Runtime。状态：首期内核全链路可测（101 runtime tests + 292 unit 全绿），多阶段 Workflow（`workflow/`）已落地定义/执行器/编排。**2026-09-03 接线落地（直接替换，无灰度开关）**：expert 会话直接走 agents 技能包路径——`chat.py._run_turn_events` 按 `assistant_target=expert` 分叉到 `_run_agent_turn_events`（AgentAssembler 装配技能包 → `PhaseExecutionRequest.system_prompt` 注入 AGENT.md+SKILL.md → 单阶段 Bounded ReAct → 事件输出），通用对话保留直接流式。模块命名去掉版本前缀：`v2_runner.py`→`agent_runner.py`（`V2Assembler`→`AgentAssembler`、`run_v2_turn`→`run_agent_turn`）、`v2_stream.py`→`run_stream.py`；Agent 预算字段 `agent_max_*` / `agent_context_summary_*` 进配置。**代码中禁止任何地方出现 `v2` 开头的命名**（V1 已删、该内核即唯一正式实现） | 用户决策 2026-09-03，react-runtime-v2-architecture §1/§3/§22 |
| **chat-messages 全量迁移 V2（2026-09-04）** | `POST /chat-messages` 全链路接入 v2：路由层 `chat.py` 只保留 HTTP 接入（路由 + `_blocking`/`_stream_sse`/`_persist_turn`），回合执行下沉 `chat_execution.py`（`RunService.create` + `QueueEmitter` + `ThinkingStarted` → 专家路径 `run_agent_turn` / 通用路径直接流式 → 终态事件）；SSE 输出统一经 `run_stream.map_run_event` （RunEvent → SSE 帧，终态帧合并 usage/assistant）；落库由 `domain/conversation/recorder.py` （TurnRecorder 监听 RunEvent，终态经 `resolve_turn_status` 映射）。**V1 全量删除**：engine.py/transition.py/state.py/context.py/execution/{action,observation,executor}.py/events/events.py（AgentEvent）与路由层内联聚合 `_TurnCollector`/`_resolve_status`/`_map_stream_event` 移除，无灰度开关、无回退 | 用户决策 2026-09-04，实测验证：ruff/mypy 全绿、tests/unit 248 通过 |
| **思考有正文无修复（2026-09-04）** | 专家会话只出思考不出正文的根因是 ReAct 触达硬预算（`agent_max_total_tokens=100000` / `agent_max_input_tokens=80000`）→ `PARTIAL_BUDGET` → 空 answer。修复六件套：① 默认预算上调（150000/120000，`.env`/`.env.example` 同步）；② MCP 泛化分发工具（`call_tool`/`call_tools_batch`）经 `mcp_hidden_tool_names` 从模型可见清单剔除（同服务存在具体工具时；tyc_mcp 唯一入口保留，2026-09-08 起）；③ `role="tool"` 回传正文按 `agent_tool_message_max_chars` 截断；④ `PhaseExecutionRequest.thinking_enabled` 由全局开关与装配层 `thinking_preference` 共同决定，`off` 时关闭思考；⑤ `check_budget` 兑现 `finalization_reserve_tokens`（token 维度进入预留区即软性收尾，不再硬截断）；⑥ TurnRecorder 区分 `succeeded`/`partial` 落库（`MessageStatus.PARTIAL`），`message_end` 透出 status/reason/limitations | 用户决策 2026-09-04，实测验证：ruff/mypy 全绿、tests/unit 324 通过 |
...
| **阶段转换结构闸门 + decide_repair 接线（2026-09-15）** | 结构契约校验从 `verification.py` 抽为公共模块 `contracts/structural.py`（`structural_failures`：required + properties.type；`has_structural_contract` 判定是否声明契约），Verifier（终点兜底）与 WorkflowRunner（过程闸门）共用同一实现与同一 schema 源（`outcome.output_schema`）。`WorkflowRunner.run` 的 CANDIDATE_COMPLETED 分支在固化 `PhaseOutput` 前插入闸门，复用此前死代码 `decide_repair` 做有界修复：RETRY→同阶段带修复说明重跑、DEGRADE→跳 `fallback_phase`、TERMINATE→终止（reason_code=`contract_failed:{phase_id}`），修复预算沿用 `max_repair_attempts`。空 schema 不校验（不误伤无契约阶段）；`StructuralContractExecutor`（contract_refs 的 required_fields/require_artifacts）与之正交不合并；TERMINATE 的 final_outcome 仍为 CANDIDATE_COMPLETED，终态由 Verifier 兜底判定。output-gate 同日收敛：`_enforce_output_contracts` 删手写 `for attempt` 循环，聚合 N 个 gate 结果为单个 `ContractResult` 喂 `decide_repair`，失败 reason_code 统一为 `contract_failed:{refs}`（`output_contract_failed:` 废弃），与结构闸门共享 `_repair_goal` | 用户决策 2026-09-15，ruff/mypy 全绿、tests/unit 631 通过（1 环境性失败除外） |
| **运行时边界 P1 收口（2026-09-15）** | 依据边界评审逐项收口：① **Context 边界拆分（P1#9）**——`ContextAssembler`/`ContextProjector` 从 `environment/context` 迁至 `harness/context`（取舍/裁剪/压缩/投影属 Harness 上下文编译器），`environment/context` 只保留事实模型 + 来源端口（回答「系统有哪些事实」）；② **Harness 解耦具体环境（P1#10）**——新增 `harness/contracts/capability.py`（`ToolBrokerPort`/`CapabilityActivatorPort`），`AgentAssembler` 不再直接实例化 ToolBroker/WorkspaceManager，具体装配下沉 `application/agent/capability.py`（CapabilityActivator：工作区创建 + ToolBroker 激活），`AssemblyResult.workspace` 收窄为 `workspace_root: Path`；③ **gate 脚本收口**——`_ToolGateRunner` 改经 `ActionGateway.execute_batch` 执行，消除 harness 层直连 ToolRunner 的第二条 `broker.execute` 旁路；④ **resume 轻量接线 + 最小 action 完成校验**——新增 `POST /chat-messages/{id}/runs/{run_id}/resume`（重新获取租约 + 快照/事件 + `pending_actions`），`RunService.pending_actions` 读副作用 action 检查点暴露「已计划未落 executed/failed」动作，供 resume 后判断是否安全续传；不做基于 snapshot/action checkpoint 的整图恢复（强恢复留待后续扩展 checkpoint） | 用户决策 2026-09-15，ruff/mypy 全绿、tests/unit 通过（1 环境性失败：缺 `agent_package_templates/` 模板目录，与本改动无关） |
...
`response_mode=streaming` 开 SSE / `blocking` 返回 JSON → Run 执行（解析助手 → 解析技能 →
装配 → 推理 ⇄ 工具）→ 收尾 RunCompleted/RunFailed 终态事件 → 流尾排空 emitter。会话 ID 经响应头
`X-Conversation-Id` 与消息体返回。进行中回合可经 `POST /chat-messages/{id}/stop`
显式取消（task.cancel() == 客户端断连原语，interrupted 落库保留 partial）。
