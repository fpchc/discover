...
> 解析器归 harness/resolver、新增 harness/execution、extensions 摊平单文件 + 访问器归位、
...
| 职责 | 路径 |
|------|------|
| frontmatter 模型（AgentManifest + SkillManifest + 声明类 + SkillWorkflowDefinition） | `app/domain/skill/manifest.py` |
...
| 技能包运行时对齐契约（AGENT/SKILL 正文边界校验，告警不阻断） | `app/domain/skill/contract.py` |
| 两级索引（智能体/技能） | `app/domain/skill/index.py` |
| 技能装配（上下文注入 + 门禁脚本注册 + workflow 透传，AssemblyPlan/SkillAssembler） | `app/domain/skill/assemble.py` |
...
| 职责 | 路径 |
|------|------|
| Run/Phase/Budget/Progress/Termination 状态模型 + P0-2 接缝（PhaseExecutionRequest/Outcome/PhaseOutput） | `app/harness/models.py` |
| Agent 解析/技能装配/执行入口（AgentAssembler/build_agent_budget/run_agent_turn/run_skill_workflow） | `app/harness/agent_runner.py` |
| 结构化决策模型与归一（三个保留控制工具 + parse_decision 五规则，P0-1） | `app/harness/react/decision.py` |
| Bounded ReAct 执行器节点逻辑（ReactGraphState/BoundedReActExecutor + Protocol 端口） | `app/harness/react/executor.py` |
| Action/Observation 指纹 + §12.4 六条件无进展判定 | `app/harness/react/progress.py` |
| PolicyDecision 模型（7 种结构化枚举） | `app/harness/policy/models.py` |
| 预算 Policy（软/硬边界判定） | `app/harness/policy/budget.py` |
| Action Policy（存在性/白名单/schema/重复无进展） | `app/harness/policy/action.py` |
| Observation Policy（空返回/重复/错误/降级） | `app/harness/policy/observation.py` |
| 组合 Policy（按严重度取最高） | `app/harness/policy/composite.py` |
| Contract 定义/结果模型（三类 + PASS/WARN/FAIL） | `app/harness/contracts/models.py` |
| ContractExecutor（structural/quality/evidence + ScriptGate 统一 Gate） | `app/harness/contracts/executor.py` |
...
| Run 生命周期事件集（RunEvent 18 类 + 终态契约） | `app/harness/events/run_events.py` |
...
| 阶段执行器注册表（ReactPhaseExecutor + RenderPhaseExecutor + PhaseExecutorRegistry） | `app/harness/workflow/executors.py` |
| Workflow 编排（WorkflowRunner 顺序阶段推进 + input_bindings 绑定） | `app/harness/workflow/compiler.py` |
| 生产适配器（LLMRunner/ToolRunner/SSEEventSink：抽象端口 → 真实运行组件） | `app/harness/wiring.py` |
