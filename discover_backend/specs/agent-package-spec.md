...
本文件定义“智能体包允许声明什么”。清单加载后的装配行为见 `skill-assembly-spec.md`；当前实现路径以 `docs/MODULE_MAP.md` 为准。
...
加载器还应对当前单阶段 Bounded ReAct 做运行时对齐告警（不阻断加载）：

- AGENT / SKILL 正文不得硬编码具体 ``<server>.<tool>`` 引用
- AGENT 正文不得出现保留控制工具（submit_final_answer / complete_phase / request_clarification）
- SKILL 正文必须声明最终提交契约（submit_final_answer 或 complete_phase）
...
- [ ] AGENT / SKILL 正文无 ``<server>.<tool>`` 引用
- [ ] AGENT 正文无保留控制工具，SKILL 正文声明 submit_final_answer / complete_phase
